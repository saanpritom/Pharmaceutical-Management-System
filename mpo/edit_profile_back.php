...
if($notification==1){
	...
	$query="SELECT first_name, last_name, company_name, ut_number, company_address, city, state, zip, country, tel_number, gender, designation FROM mpo_details WHERE mpo_id='$sm_id'";
				...
				$query = "SELECT username, password FROM mpo_login WHERE mpo_id='$sm_id'";
						$result=mysql_query($query,$dbs);
						while($row=mysql_fetch_array($result)){
					...
					$query="UPDATE mpo_details SET first_name='$first_name', last_name='$last_name', company_address='$com_address', city='$city', state='$state', zip='$zip', country='$county', tel_number='$tel_numb',
								designation='$designation', gender='$webs' WHERE mpo_id='$sm_id'";
					if(mysql_query($query,$dbs)){
						$query="UPDATE mpo_login SET username='$username', password='$password' WHERE mpo_id='$sm_id'";
						if(mysql_query($query,$dbs)){
