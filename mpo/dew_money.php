					...
					<table class="table table-responsive">
						...
						<tbody>
							<?php
								$count=1;
								$o_id=0;
								$query="SELECT cus_id FROM order_relation WHERE mpo_id='$sm_id'";
								$result=mysql_query($query,$dbs);
								while($row=mysql_fetch_array($result)){
									$cus_id=$row['cus_id'];
									
									$query1="SELECT bill_status.order_id, bill_status.total_bill, customer_details.company_name FROM bill_status INNER JOIN order_relation ON bill_status.order_id=order_relation.order_id
												INNER JOIN customer_details ON order_relation.cus_id=customer_details.cus_id WHERE bill_status.pay_status='0' and customer_details.cus_id='$cus_id'";
