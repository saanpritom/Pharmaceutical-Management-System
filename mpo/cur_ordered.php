						...
						<tbody>
							<?php
								...
								$query="SELECT product_description.business_name, product_description.scientific_name, product_description.product_type, order_relation.cus_id, order_relation.order_id, 
										...
										INNER JOIN order_status ON order_relation.order_id=order_status.order_id WHERE order_relation.mpo_id='$sm_id' and order_status.status='1'";
								$result=mysql_query($query,$dbs);
								while($row=mysql_fetch_array($result)){
									?>
									<tr>
										...
										<td><a href="approve_order.php?order_id=<?php echo $row['order_id'];?>" type="button" class="btn btn-warning">Approve It.</a></td>
									</tr>
									<?php
									$count++;
								}
