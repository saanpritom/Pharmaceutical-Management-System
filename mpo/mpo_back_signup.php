...
<nav class="navbar navbar-default navbar-fixed-bottom">
	...
	$query="SELECT email FROM mpo_login WHERE ut_num='$ut_num'";
	...
	if($f_name!=null){
		...
		$query = "SELECT MAX(mpo_id) AS max_id FROM mpo_details WHERE 1";
    $result = mysql_query($query,$dbs);
    while($row=mysql_fetch_array($result)){
        $max_id = $row['max_id'];
    }
    ...
    $query = "INSERT INTO mpo_login (mpo_id, username, email, password) VALUES ('$max_id', '$username','$email','$password')";
    mysql_query($query, $dbs);

    $query = "INSERT INTO `mpo_details`(`mpo_id`, `first_name`, `last_name`, `company_name`, `ut_number`, `company_address`, `city`, `state`, `zip`, `country`, `tel_number`, `gender`, `designation`) VALUES 
