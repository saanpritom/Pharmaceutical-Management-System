			...
			<div class="col-md-12">
				<div class="col-md-4">
					...
					<?php
						...
						$mpo_id = $_GET['mpo_id'];
						$flag=0;
						$query="SELECT status FROM mpo_customer_relation WHERE cus_id='$mpo_id' and mpo_id='$sm_id'";
						$result=mysql_query($query, $dbs);
						while($row=mysql_fetch_array($result)){
							$status = $row['status'];
							if($status==1){
						...
						if($flag==2){
							?>
								<div class="alert alert-warning">
									<strong>Sorry!</strong> But he is already in your network.<a href="customer_list.php">Go Back</a>
								</div>
							<?php
						}
						else if($flag==1){
							?>
								<div class="alert alert-warning">
									<strong>Wait!</strong>Request is still pending.<a href="customer_list.php">Go Back</a>
								</div>
							<?php
						}
						else{
							$query="INSERT INTO `mpo_customer_relation`(`mpo_id`, `cus_id`, `status`) VALUES ('$sm_id','$mpo_id','1')";
							if(mysql_query($query, $dbs)){
								?>
								<div class="alert alert-success">
									<strong>Successful </strong>But you have to wait until that request will be accepted.<a href="customer_list.php">Go Back</a>
								</div>
							<?php
							}
						}
