			...
			<div class="col-md-12">
				<div class="col-md-4">
					...
					<?php
						...
						$query="SELECT sm_details.sm_id FROM mpo_details INNER JOIN sm_details ON sm_details.ut_num=mpo_details.ut_number WHERE mpo_details.ut_number='$ut_num'";
						$result=mysql_query($query,$dbs);
						while($row=mysql_fetch_array($result)){
							$s_id = $row['sm_id'];
						}
						
						$query="SELECT MAX(order_id) AS max_id FROM order_relation WHERE 1";
						$result=mysql_query($query,$dbs);
						while($row=mysql_fetch_array($result)){
							$max_id = $row['max_id'];
						}
						...
						$query="INSERT INTO `order_relation`(`order_id`, `product_id`, `mpo_id`, `cus_id`, `ut_number`) VALUES ('$max_id', '$pro_id','$mpo_id','$sm_id','$ut_num')";
						if(mysql_query($query,$dbs)){
							$query="INSERT INTO `order_status`(`order_id`, `status`) VALUES ('$max_id','1')";
							if(mysql_query($query,$dbs)){
								$query="INSERT INTO `order_description`(`order_id`, `quantity`, `price`, `date_of_order`) VALUES ('$max_id','$quantity','$price','$date')";
								if(mysql_query($query, $dbs)){
									?>
									<p class="alert alert-success">Product Ordered Successfully</p>
									<?php
								}
								else{
									?>
									<p class="alert alert-danger">Some Problem Happened 1</p>
									<?php
								}
							}
							else{
									?>
									<p class="alert alert-danger">Some Problem Happened 2</p>
									<?php
								}
						}
						else{
									?>
									<p class="alert alert-danger">Some Problem Happened 3</p>
									<?php
								}
					?>
