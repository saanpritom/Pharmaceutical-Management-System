			...
			<div class="col-md-12">
				<div class="col-md-6">
					<div class="well">
						<?php
							$pro_id = $_GET['pro_id'];
							$query="SELECT business_name, scientific_name, product_type, batch_number, price FROM product_description WHERE product_id='$pro_id'";
							$result=mysql_query($query,$dbs);
							while($row=mysql_fetch_array($result)){
							?>
							<img src="../sales_manager/database_images/product_images/<?php echo $pro_id; ?>.jpg" width="360px" height="230px"><br><br><br>
							<strong>Product Name     </strong><?php echo $row['business_name']; ?> <br><br>
							<strong>Scientific Name  </strong><?php echo $row['scientific_name']; ?><br><br>
							<strong>Product Type     </strong><?php echo $row['product_type']; ?><br><br>
							<strong>Batch Number     </strong><?php echo $row['batch_number']; ?><br><br>
							<strong>Price     </strong><?php $price=$row['price']; echo $row['price']; ?><br><br>
							<?php
								}
				...
				<div class="col-md-6">
					<div class="well">
						...
						<form action="order_product.php" method="POST">
							...
							<label>Select the MPO of the company</label>
							<select class="form-control" name="mpo_id">
								<?php
									
									$query="SELECT ut_num FROM company_product WHERE product_id='$pro_id'";
									$result=mysql_query($query,$dbs);
									while($row=mysql_fetch_array($result)){
										$ut_num = $row['ut_num'];
									}
									...
									$query="SELECT mpo_details.mpo_id, mpo_details.first_name, mpo_details.last_name FROM mpo_details INNER JOIN mpo_customer_relation ON 
											mpo_details.mpo_id=mpo_customer_relation.mpo_id WHERE (mpo_customer_relation.cus_id='$sm_id' and mpo_customer_relation.status='2') and mpo_details.ut_number='$ut_num'";
											
									$result=mysql_query($query,$dbs);
									while($row=mysql_fetch_array($result)){
										?>
										<option value="<?php echo $row['mpo_id']; ?>"><?php echo $row['first_name']; echo ' '; echo $row['last_name']; ?></option>
										<?php
									}
							...
							<input type="submit" class="btn btn-primary" value="Order IT">
							<?php
