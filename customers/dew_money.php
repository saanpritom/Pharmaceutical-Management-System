					...
					<table class="table table-responsive">
						...
						<tbody>
							<?php
								$count=1;
								$o_id=0;
								$query="SELECT mpo_id FROM order_relation WHERE cus_id='$sm_id'";
								$result=mysql_query($query,$dbs);
								while($row=mysql_fetch_array($result)){
									$mpo_id=$row['mpo_id'];
									
									$query1="SELECT bill_status.order_id, bill_status.total_bill, mpo_details.company_name FROM bill_status INNER JOIN order_relation ON bill_status.order_id=order_relation.order_id
												INNER JOIN mpo_details ON order_relation.mpo_id=mpo_details.mpo_id WHERE bill_status.pay_status='0' and mpo_details.mpo_id='$mpo_id'";
									
									$result1=mysql_query($query1,$dbs);
									while($row1=mysql_fetch_array($result1)){
										if($o_id!=$row1['order_id']){
											?>
										<tr>
											<td><?php echo $count;?></td>
											<td><?php echo $row1['order_id'];?></td>
											<td><?php echo $row1['company_name'];?></td>
											<td><?php echo $row1['total_bill'];?></td>
											<td><a href="dew_pay.php?order_id=<?php echo $row1['order_id']; ?>" type="button" class="btn btn-success">Pay it</a></td>
										</tr>
										<?php
										}
