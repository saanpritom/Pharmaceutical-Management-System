		...
		<div class="row">
			<div class="col-md-12">
				...
				<?php
					...
					$username=$_POST['mpo'];
					$flag=0;
					$query="SELECT mpo_id FROM mpo_login WHERE username='$username'";
					...
					if($flag==1){
							$query="SELECT first_name, last_name, company_name FROM mpo_details WHERE mpo_id='$mpo_id'";
							$result=mysql_query($query, $dbs);
							while($row=mysql_fetch_array($result)){
								?>
									<form action="add_mpo_back.php" method="POST">
										<div class="well center-block">
											<img src="../mpo/database_images/mpo_images/<?php echo $mpo_id; ?>.jpg" class="img-responsive" alt="Responsive image" width="30%" height="auto"><br>
											<?php echo $row['first_name']; echo "   "; echo $row['last_name']; ?><br><br>
											<?php echo $row['company_name']; ?><br>
											<a href="add_mpo_back.php?mpo_id=<?php echo $mpo_id; ?>" type="button" class="btn btn-primary">Connect to your network</a>
										</div>
									</form>
								<?php
							}
						...
						if($flag==0){
							?>
								<div class="alert alert-danger">
									<strong>Sorry!</strong> No such MPO Found.<a href="mpo_list.php">Go Back</a>
								</div>
							<?php
						}
				?>
