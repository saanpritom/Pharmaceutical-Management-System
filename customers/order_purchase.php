				...
				<div class="table">
					<table class="table table-responsive">
						<thead>
							...
							<th>Total Money Transaction</th>
						</thead>
						<tbody>
							<?php
								$count=1;
								
								$query="SELECT DISTINCT mpo_id FROM order_relation WHERE cus_id='$sm_id'";
								$result=mysql_query($query,$dbs);
								while($row=mysql_fetch_array($result)){
									$mpo_id=$row['mpo_id'];
									$query1="SELECT SUM(order_description.quantity) AS t_quantity, SUM(order_description.price) AS t_price, mpo_details.company_name FROM order_description INNER JOIN order_relation ON 
												order_description.order_id=order_relation.order_id INNER JOIN mpo_details ON order_relation.mpo_id=mpo_details.mpo_id WHERE order_relation.mpo_id='$mpo_id'";
									...
									while($row1=mysql_fetch_array($result1)){
										...
										<tr>
											...
											<td><?php echo $row1['t_quantity'];?></td>
											<td><?php echo $row1['t_quantity']*$row1['t_price'];?></td>
										</tr>
										<?php
									}
									$count++;
