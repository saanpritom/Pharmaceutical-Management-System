						...
						<tbody>
							<?php
								...
								$query="SELECT product_description.business_name, product_description.scientific_name, product_description.product_type, order_relation.mpo_id, order_relation.order_id, 
										...
										INNER JOIN order_status ON order_relation.order_id=order_status.order_id WHERE order_relation.cus_id='$sm_id' and order_status.status='1'";
								$result=mysql_query($query,$dbs);
								while($row=mysql_fetch_array($result)){
									?>
									<tr>
										...
										<td style="background-color:red; color:white;">Not Approved Yet</td>
									</tr>
									<?php
									$count++;
								}
