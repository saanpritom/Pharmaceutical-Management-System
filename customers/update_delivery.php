				...
				<div class="col-md-6">
					<div class="well">
						<?php
							...
							$query="SELECT cus_id FROM order_relation WHERE order_id='$order_id'";
							$result=mysql_query($query,$dbs);
							while($row=mysql_fetch_array($result)){
								$cus_id=$row['cus_id'];
							}
							...
							if($cus_id!=$sm_id){
								?>
									<h2 style="color:red;">This order number doesn't belong to you. Please try again. <a href="delivered.php">Try again</a></h2>
								<?php
							}
							else{
								$query="SELECT product_id FROM order_relation WHERE order_id='$order_id'";
								$result=mysql_query($query,$dbs);
								while($row=mysql_fetch_array($result)){
									$pro_id = $row['product_id'];
								}
								
								$query="SELECT business_name, scientific_name, product_type, batch_number FROM product_description WHERE product_id='$pro_id'";
								$result=mysql_query($query,$dbs);
								while($row=mysql_fetch_array($result)){
									?>
									<ol align="center"><img src="../sales_manager/database_images/product_images/<?php echo $pro_id; ?>.jpg" width="170px" height="170px"></ol>
									<li align="center">Product Name: <?php echo $row['business_name']; ?></li>
									<li align="center">Scientific Name: <?php echo $row['scientific_name']; ?></li>
									<li align="center">Product Type: <?php echo $row['product_type']; ?></li>
									<li align="center">Batch Number: <?php echo $row['batch_number']; ?></li>
									<?php
								}
								
								$query="SELECT quantity, price, date_of_order FROM order_description WHERE order_id='$order_id'";
								$result=mysql_query($query,$dbs);
								while($row=mysql_fetch_array($result)){
									?>
									<li align="center">Quantity: <?php echo $row['quantity']; ?></li>
									<li align="center">Price: <?php echo $row['price']; ?></li>
									<li align="center">Total Bill: <?php echo $row['price']*$row['quantity']; ?></li>
									<?php
						...
						?>
						<hr>
						<form action="update_delivery_back.php" method="POST">
							<div class="radio">
								<label><input type="radio" name="bill_pay" value="1">Pay it</label>
							</div>
							<div class="radio">
								<label><input type="radio" name="bill_pay" value="0">Make a Dew</label>
							</div>
