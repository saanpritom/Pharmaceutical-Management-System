					...
					<?php
						...
						<div class="well" align="center">
							...
							<?php
								...
								$query="SELECT first_name, last_name, company_name, city, state, zip, country FROM customer_details WHERE cus_id='$cus_id'";
								$result=mysql_query($query,$dbs);
								while($row=mysql_fetch_array($result)){
									?>
									...
									<?php
									echo '<br><br>';
									echo 'Name   '; echo $row['first_name']; echo ' '; echo $row['last_name']; echo '<br>';
									echo 'Organaization Name   '; echo $row['company_name'];echo '<br>';
									echo 'City   '; echo $row['city'];echo '<br>';
									echo 'State   '; echo $row['state'];echo '<br>';
									echo 'Zip   '; echo $row['zip'];echo '<br>';
									echo 'Country   '; echo $row['country'];echo '<br>';
								}
							
							?>
							<br><br>
							<strong>Connected MPO</strong><br>
							<?php
								...
								$query="SELECT mpo_details.mpo_id, mpo_details.first_name, mpo_details.last_name FROM mpo_details INNER JOIN mpo_customer_relation ON 
											mpo_details.mpo_id=mpo_customer_relation.mpo_id 
											WHERE (mpo_customer_relation.cus_id='$cus_id' and mpo_customer_relation.status='2') and mpo_details.ut_number='$ut_num'";
								
								$result=mysql_query($query,$dbs);
								while($row=mysql_fetch_array($result)){
									?>
									<p class="alert alert-success"><a href="mpo_details.php?mpo_id=<?php echo $row['mpo_id']; ?>"><?php echo $row['first_name']; echo ' '; echo $row['last_name']; ?></a></p><br>
									<?php
								}
								...
								<tbody>
										...
										<td>
											...
											<?php
												...
												$query="SELECT SUM(bill_status.total_bill) as t_bill FROM bill_status INNER JOIN order_relation ON bill_status.order_id=order_relation.order_id
														WHERE order_relation.cus_id='$cus_id' and order_relation.ut_number='$ut_num'";
									...
									<tr>
										...
										<?php
												...
												$query="SELECT SUM(bill_status.total_bill) as t_bill FROM bill_status INNER JOIN order_relation ON bill_status.order_id=order_relation.order_id
														WHERE (order_relation.cus_id='$cus_id' and order_relation.ut_number='$ut_num') and bill_status.pay_status='1'";
									...
									<tr>
										<td>Connected MPO of this Company</td>
										<td>
										
										<?php
										
											$query="SELECT COUNT(mpo_customer_relation.mpo_id) AS t_id FROM mpo_customer_relation INNER JOIN mpo_details ON
													mpo_customer_relation.mpo_id=mpo_details.mpo_id WHERE mpo_customer_relation.cus_id='$cus_id'";
											$result=mysql_query($query,$dbs);
											while($row=mysql_fetch_array($result)){
												echo $row['t_id'];
											}
