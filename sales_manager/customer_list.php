											...
											<table class="table table-responsive table-striped table-hover">
												...
												<tbody>
													<?php
														...
														$query="SELECT mpo_id FROM mpo_details WHERE ut_number='$ut_num'";
														$result=mysql_query($query,$dbs);
														while($row=mysql_fetch_array($result)){
															$mpo_id = $row['mpo_id'];
															$count2=1;
															$query2="SELECT customer_details.cus_id, customer_details.first_name, customer_details.last_name, customer_details.company_name, customer_details.tel_number FROM customer_details
																		INNER JOIN mpo_customer_relation ON mpo_customer_relation.cus_id=customer_details.cus_id WHERE mpo_customer_relation.mpo_id='$mpo_id' and mpo_customer_relation.status='2'";
															$result2=mysql_query($query2, $dbs);
															while($row2=mysql_fetch_array($result2)){
															?>
															<tr>
																<td><?php echo $count2; ?></td>
																<td><img src="../customers/database_images/mpo_images/<?php echo $row2['cus_id']; ?>.jpg" width="60px" height="60px"></td>
																<td><a href="customer_details.php?cus_id=<?php echo $row2['cus_id']; ?>"><?php echo $row2['first_name']; ?> </a></td>
																<td><a href="customer_details.php?cus_id=<?php echo $row2['cus_id']; ?>"><?php echo $row2['last_name']; ?> </a></td>
																<td><?php echo $row2['company_name'] ?></td>
																<td><?php echo $row2['tel_number'] ?></td>
															</tr>
															<?php
															$count2++;		
