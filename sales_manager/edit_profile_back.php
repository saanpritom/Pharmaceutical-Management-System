...
if($notification==1){
	...
	$query="SELECT first_name, last_name, com_name, ut_num, com_address, city, state, zip, country, tel_numb, designation, webs FROM sm_details WHERE sm_id='$sm_id'";
				...
				$query = "SELECT username, password FROM sm_login WHERE sm_id='$sm_id'";
					...
					$query="UPDATE sm_details SET first_name='$first_name', last_name='$last_name', com_address='$com_address', city='$city', state='$state', zip='$zip', country='$county', tel_numb='$tel_numb',
								designation='$designation', webs='$webs' WHERE sm_id='$sm_id'";
					if(mysql_query($query,$dbs)){
						$query="UPDATE sm_login SET username='$username', password='$password' WHERE sm_id='$sm_id'";
						if(mysql_query($query,$dbs)){
