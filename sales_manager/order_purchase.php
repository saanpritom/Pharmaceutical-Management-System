			...
			<div class="col-md-12">
				<div class="col-md-6">
					...
					<table class="table table-responsive">
						<strong>Pharmasist Info</strong>
						<thead>
							<th>Sl. No.</th>
							<th>Pharmasist Image</th>
							<th>Pharmasist Name</th>
							<th>Organaization Name</th>
							<th>Ordered Products Number</th>
						...
						<tbody>
							<?php
								...
								$query="SELECT customer_details.cus_id, customer_details.first_name, customer_details.last_name, customer_details.company_name, SUM(order_description.price) AS t_bill,
											SUM(order_description.quantity) AS t_quantity FROM customer_details INNER JOIN order_relation ON customer_details.cus_id=order_relation.cus_id INNER JOIN bill_status
											ON order_relation.order_id=bill_status.order_id INNER JOIN order_description ON order_relation.order_id=order_description.order_id WHERE order_relation.ut_number='$ut_num'";
								$result=mysql_query($query,$dbs);
								while($row=mysql_fetch_array($result)){
									?>
									<tr>
										<td><?php echo $count; ?></td>
										<td><img src="../customers/database_images/mpo_images/<?php echo $row['cus_id']; ?>.jpg" width="70px" height="70px"></td>
										<td><?php echo $row['first_name']; echo '  '; echo $row['last_name']; ?></td>
										<td><?php echo $row['company_name']; ?></td>
										<td><?php echo $row['t_quantity']; ?></td>
										<td><?php echo $row['t_bill']*$row['t_quantity']; ?></td>
									</tr>
									<?php
									$count++;
								}
					...
					<table class="table table-responsive">
						...
						<tbody>
							<?php
								...
								$query="SELECT mpo_details.mpo_id, mpo_details.first_name, mpo_details.last_name, mpo_details.company_name, SUM(order_description.price) AS t_bill,
											SUM(order_description.quantity) AS t_quantity FROM mpo_details INNER JOIN order_relation ON mpo_details.mpo_id=order_relation.mpo_id INNER JOIN bill_status
											ON order_relation.order_id=bill_status.order_id INNER JOIN order_description ON order_relation.order_id=order_description.order_id WHERE order_relation.ut_number='$ut_num'";
								$result=mysql_query($query,$dbs);
								while($row=mysql_fetch_array($result)){
									?>
									<tr>
										<td><?php echo $count; ?></td>
										<td><img src="../mpo/database_images/mpo_images/<?php echo $row['mpo_id']; ?>.jpg" width="70px" height="70px"></td>
										<td><?php echo $row['first_name']; echo '  '; echo $row['last_name']; ?></td>
										<td><?php echo $row['company_name']; ?></td>
										<td><?php echo $row['t_quantity']; ?></td>
										<td><?php echo $row['t_bill']*$row['t_quantity']; ?></td>
