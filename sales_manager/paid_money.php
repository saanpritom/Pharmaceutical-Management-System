					...
					<table class="table table-responsive">
						...
						<tbody>
							<?php
								...
								$count=1;
								$query="SELECT customer_details.cus_id, customer_details.first_name, customer_details.company_name, bill_status.total_bill FROM customer_details INNER JOIN
										order_relation ON customer_details.cus_id=order_relation.cus_id INNER JOIN bill_status ON order_relation.order_id=bill_status.order_id WHERE 
										order_relation.ut_number='$ut_num' and bill_status.pay_status='1'";
								$result=mysql_query($query,$dbs);
								while($row=mysql_fetch_array($result)){
									?>
									<tr>
				...
				<div class="col-md-6">
					...
					<table class="table table-responsive">
						...
						<thead>
							...
							<th>First Name</th>
							<th>Last Name</th>
							<th>Market Withdraw</th>
						</thead>
						<tbody>
							<?php
								$count=1;
								
								$query="SELECT mpo_details.mpo_id, mpo_details.first_name, mpo_details.last_name, bill_status.total_bill FROM mpo_details INNER JOIN
										order_relation ON mpo_details.mpo_id=order_relation.mpo_id INNER JOIN bill_status ON order_relation.order_id=bill_status.order_id WHERE 
										order_relation.ut_number='$ut_num' and bill_status.pay_status='1'";
								$result=mysql_query($query,$dbs);
								while($row=mysql_fetch_array($result)){
									?>
